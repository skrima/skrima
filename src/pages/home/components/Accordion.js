  ...
  return (
    <Fragment>
        <div className="accordion-item">
            <div className={expanded ? "accordion-title show" : "accordion-title"} onClick={() => setExpanded(!expanded)}>
                <h2>{title}</h2>
                {expanded ? <FaMinus /> : <FaPlus />}
            </div>
            <div className="accordion-item__line"/>
            <div className={expanded ? 'accordion-content show' : 'accordion-content'}>
                {
                    content.map((x, i) => {
                        return (
